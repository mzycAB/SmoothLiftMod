 ...
 *
 * <p>【1.44】起这个插件同时管**多条** mixin，门禁各自独立（见 {@link #shouldApplyMixin}）：
 * <ul>
 *   <li>{@link Mtr3LiftDoorMixin} —— 只在 MTR3 生效；</li>
 *   <li>【1.45】{@link Mtr3LiftExternalCallMixin}（外呼入口）、{@link Mtr3LiftPanelCallMixin}
 *       （轿厢面板入口）—— 同样只在 MTR3 生效（它们的目标 {@code mtr.data.LiftInstructions} /
 *       {@code mtr.data.Lift} 都只在 MTR3 存在）；</li>
 *   <li>{@link Mtr4LiftTrackFloorShapeMixin} —— 只在 MTR4 生效
 *       （按它的目标类 {@code org.mtr.mod.block.BlockLiftTrackFloor} 是否存在判定）。</li>
 *   <li>【1.49】{@link Mtr4LiftTrackFloorShapeMixinV2} —— MTR **4.1.x** 的楼层轨道
 *       （4.1 把方块类挪到了 {@code org.mtr.block.BlockLiftTrackFloor}，旧 mixin 的目标类
 *       在 4.1 上不存在会被静默跳过 ⇒ 碰撞箱隐形墙复发，用户 1.21.x 上报「X光般透明」）。
 *       门禁独立：只按新包名 {@code org.mtr.block.BlockLiftTrackFloor} 是否存在判定。</li>
 * </ul>
 * 判据就是「类名不以 {@code Mtr4LiftTrackFloorShapeMixin} 结尾的一律按 MTR3 门禁走」，
 * 所以【1.45】新加的两条不用改这个插件。
 * ★ 注意判别名后缀的顺序：{@code Mtr4LiftTrackFloorShapeMixinV2} 也以
 * {@code Mtr4LiftTrackFloorShapeMixin} 结尾，必须先判 V2 再判旧名（见 {@link #shouldApplyMixin}）。
 *
...
public class Mtr3LiftMixinPlugin implements IMixinConfigPlugin {
        ...
        if (mtr3 == null) {
            ...
            if (mtr3) {
                LOGGER.info("[SmoothLift/Mtr3Fix] 检测到 MTR 3（mtr.data.LiftServer）"
                        + "—— 直梯自动关门修复 +「同一层外呼也能开门」修复已启用");
            } else {
                LOGGER.info("[SmoothLift/Mtr3Fix] 未启用 MTR 3 直梯修复"
    ...
    private static boolean mtr4TrackFloorPresent() {
        if (mtr4TrackFloor == null) {
            ...
            LOGGER.info("[SmoothLift/Mtr3Fix] 楼层轨道碰撞箱修复（MTR4 4.0.x，org.mtr.mod.block）：{}",
                    mtr4TrackFloor ? "目标类在，已启用" : "目标类不在（4.1 或没装 MTR4），跳过");
        }
        return mtr4TrackFloor;
    }

    /**
     * 【1.49】MTR **4.1.x** 的楼层轨道方块类是否在（{@code org.mtr.block.BlockLiftTrackFloor}）。
     * 4.1 把方块类从 {@code org.mtr.mod.block} 挪到了 {@code org.mtr.block}，旧 mixin 的
     * 目标类在 4.1 上不存在 ⇒ 旧门禁为 false、碰撞箱隐形墙复发（用户 1.21.x 报的
     * 「X光般透明」）。这里按**新目标类本身**判定，与旧门禁互不干扰。
     */
    private static Boolean mtr4TrackFloorV2;

    private static boolean mtr4TrackFloorV2Present() {
        if (mtr4TrackFloorV2 == null) {
            mtr4TrackFloorV2 = classPresent("org.mtr.block.BlockLiftTrackFloor");
            LOGGER.info("[SmoothLift/Mtr3Fix] 楼层轨道碰撞箱修复（MTR4 4.1.x，org.mtr.block）：{}",
                    mtr4TrackFloorV2 ? "目标类在，已启用" : "目标类不在（4.0.x 或没装 MTR4），跳过");
        }
        return mtr4TrackFloorV2;
    }
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        // 【1.44】本配置文件里有多条 mixin，门禁不同：
        //   Mtr3LiftDoorMixin / Mtr3LiftExternalCallMixin / Mtr3LiftPanelCallMixin
        //       只管 MTR3（目标类是 MTR3 独有）
        //   Mtr4LiftTrackFloorShapeMixin    只管 MTR4 4.0.x（按旧包名目标类是否存在判定）
        //   Mtr4LiftTrackFloorShapeMixinV2  只管 MTR4 4.1.x（按新包名目标类是否存在判定）
        // ★ 顺序不能反：V2 也以 "Mtr4LiftTrackFloorShapeMixin" 结尾，必须先判 V2。
        if (mixinClassName.endsWith("Mtr4LiftTrackFloorShapeMixinV2")) {
            return mtr4TrackFloorV2Present();
        }
