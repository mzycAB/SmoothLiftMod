...
public final class EscalatorRenderMode {
    ...
    /**
     * {@link #load()} 是否已经跑过。跑过之后不再读盘（load 是幂等的）。
     *
     * <p>★ 为什么需要这个标志：{@link EscalatorModelOverride} 会在**模型烘烤时**读
     * {@link #isOptimized()} 决定要不要注入透明标记贴图，而 Forge 的**首次资源重载**
     * 可能发生在客户端初始化事件之前 —— 那时 {@code load()} 还没被调用，
     * 若用户上次用 {@code /mtrxr on} 存过盘，第一遍烘焙就会用错模式，
     * 结果「MTR 原版渲染 + 阶梯面已被透明标记隐藏」⇒ 阶梯整片消失，
     * 要等下一次资源重载才恢复。所以在 {@link #isOptimized()} 里做一次懒加载兜底：
     * 第一次被问到就把 config 读进来，谁先来都不会读错。
     */
    ...
    public static boolean isOptimized() {
        // 懒加载兜底：模型烘烤早于客户端初始化事件时，也要读到正确的模式。
        // （load() 自己是幂等的，重复调用只会读一次盘。）
        if (!configLoaded) {
            load();
        }
        return optimized;
    ...
    public static boolean apply(boolean optimize) {
        ...
        // 【Forge】Fabric 侧这里还有一句 EscalatorAnimationDriver.clear()，但那个类
        //   （连同 EscalatorStepTicker / EscalatorSpriteTickerMixin 这一整条链）是**上一代**
        //   的阶梯动画实现，已经被「每帧一张贴图 + 静态几何缓存」取代，Forge 1.20.1 从来没
        //   移植过它 —— 它只驱动 MTR 那张已被换成全透明标记的 sprite，对可见效果是空操作。
        //   所以这里**故意没有**那一句；不要为了「看起来对齐」去补一个死类。
    ...
    private static Path configFile() {
        try {
            // 【Forge】Fabric 用 FabricLoader.getInstance().getConfigDir()，
            //   Forge 1.20.1 的等价物是 FMLPaths.CONFIGDIR.get()（都在 <游戏目录>/config/ 下）。
