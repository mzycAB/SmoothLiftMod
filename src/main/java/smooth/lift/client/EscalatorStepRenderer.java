 ...
 * {@code isVisible} 判定，没有额外分配。
 *
 * <p>阶梯动画速度取自 {@link EscalatorSpeedManager#getAnimationSpeed}：单独设置过就用单独值，
 * 否则跟随这条扶梯自己的运行速度。
 *
 * <p>Forge 版：渲染钩子用 {@code RenderLevelStageEvent}。1.19.2/1.19.4 的 Stage 枚举没有
 * AFTER_ENTITIES（1.20.4 才有），用 AFTER_TRIPWIRE_BLOCKS —— 它在实体之后、天气之前，
 * 效果与 AFTER_ENTITIES 一致（阶梯面既不遮实体、也不被透明方块误盖）。
 */
public final class EscalatorStepRenderer {
    ...
    /** FORGE 总线回调：只在 AFTER_TRIPWIRE_BLOCKS 阶段绘制。 */
