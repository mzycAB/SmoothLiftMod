...
public final class EscalatorStepRenderer {
     ...
     * 注册扶梯阶梯索引（区块事件的挂载点）。
     *
     * <p><b>Forge 版没有渲染回调要在这里注册</b>：Fabric 的
     * {@code WorldRenderEvents.AFTER_ENTITIES} 在 Forge 上对应
     * {@link RenderLevelStageEvent} 的 {@code Stage.AFTER_ENTITIES} 阶段，
     * 而它必须由 {@code @Mod.EventBusSubscriber} 的 {@code @SubscribeEvent} 方法接
     * （见 {@link SmoothLiftClientEvents#onRenderLevelStage}），没法在普通静态方法里挂。
     * 所以真正的渲染入口是 {@link #onRenderLevelStage(RenderLevelStageEvent)}。
     */
    ...
    /**
     * Forge 渲染入口：{@code RenderLevelStageEvent} 的 {@code AFTER_ENTITIES} 阶段。
     *
     * <p>与 Fabric 的 {@code WorldRenderEvents.AFTER_ENTITIES} 一一对应 ——
     * 都是「实体画完、天气/粒子之前」的那个时点，此时 {PoseStack} 仍是世界坐标系。
     */
    ...
    private static void render(RenderLevelStageEvent event) {
        ...
        // 【Forge】Fabric 的 WorldRenderContext.world() 在 Forge 上没有对应方法
        //   （RenderLevelStageEvent 只给 LevelRenderer），所以直接读 minecraft.level。
        ...
        int drawCalls = draw(event, partialTick, cameraPos);
     ...
     * @return 实际发出的绘制调用次数
     */
    private static int draw(RenderLevelStageEvent event, float partialTick, Vec3 cameraPos) {
