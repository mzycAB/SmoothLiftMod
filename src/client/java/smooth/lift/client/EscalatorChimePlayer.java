...
public final class EscalatorChimePlayer {
    ...
    public static void onClientTick(Minecraft mc) {
        ...
        update(mc, End.BOARD, target.boardPos, chimeEventFor(rates.in()), chimePitchFor(rates.in()),
                player, "上客端（进入，" + rates.in() + " 次/秒）", volumeFactor, range);
        update(mc, End.ALIGHT, target.alightPos, chimeEventFor(rates.out()), chimePitchFor(rates.out()),
                player, "落客端（离开，" + rates.out() + " 次/秒）", volumeFactor, range);
    }
     ...
     *
     * @param what         日志里用的中文名（「上客端（进入，N 次/秒）」/「落客端（离开，N 次/秒）」）
     * @param event        【1.31】该速率对应的**素材**（见 {@link #chimeEventFor}）；换素材时重建实例
     * @param pitch        【1.31】该速率对应的 playback pitch（见 {@link #chimePitchFor}，必在 [0.5,2.0] 内）
     * @param volumeFactor 【1.18】提示音音量系数（{@link #helpVolumeFactor}，100 → 1.0）
     * @param range        【1.24】这条扶梯的可闻范围（格，见 {@link #helpRange}），逐 tick 现算
     */
    private static void update(Minecraft mc, End kind, Vec3 pos, ResourceLocation event, float pitch, Vec3 player,
        ...
        if (inst != null && !inst.event.equals(event)) {
            // 【1.31】速率跨了素材档（如 8 → 9 Hz）就要换素材，只能重建实例。
