...
public final class EscalatorAudioPlayer {
    ...
    private static boolean inject(Minecraft mc, String audioId, byte[] bytes) {
        ...
        try (JOrbisAudioStream stream = new JOrbisAudioStream(new ByteArrayInputStream(bytes))) {
            ...
            // MC 用 JOrbis 解码（1.21.1；1.20.4 及更早是 stb_vorbis），只认 Ogg 容器 + Vorbis 编码：
            // MP3 改扩展名（容器头不是 OggS）、Ogg Opus/FLAC 都会在这里失败。
    ...
    private static ResourceLocation soundLocation(String audioId) {
        ...
        return ResourceLocation.fromNamespaceAndPath("smoothlift", path.toString());
    }
    ...
    private static ResourceLocation builtinEvent(String audioId) {
        return ResourceLocation.fromNamespaceAndPath("smoothlift", "audio/" + EscalatorSpeedManager.builtinKey(audioId));
    }
    ...
    private static final class EscalatorSoundInstance extends AbstractSoundInstance implements TickableSoundInstance {
        ...
        public WeighedSoundEvents resolve(SoundManager soundManager) {
            ...
            // 1.21.1 起 Sound 的第一个参数是 ResourceLocation（1.20.4 还是 String）。
