        ...
        for (Group group : GROUPS) {
            ...
            for (int i = 0; i < bucket.size(); i++) {
                ...
                try {
                    ...
                    // （{@code ModelBlockRenderer.renderModelFaceFlat}：{@code pos.relative(quad.getDirection())}），
                    ...
                    // MTR 的阶梯模型里确实有这样的面（平层/过渡块的 {@code #step} 面就带
                    // {@code cullface:up}），所以传 true 之后行为与原版逐面一致。
