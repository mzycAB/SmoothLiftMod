 ...
 *
 * <h2>为什么必须「一组 RenderType 一趟画完」（本项目踩过的大坑）</h2>
 * 阶梯面其实有**四组**（斜坡上/下、平层上/下），而且平层与斜坡是**两张不同的贴图**
 * （见 {@link EscalatorStepTextures}），所以每组必须有自己的 RenderType。
 *
 * <p>但 1.19.2 的 {@code MultiBufferSource.BufferSource} 对**没有登记进 {@code fixedBuffers}
 * 的自定义 RenderType** 只养**一个共享 BufferBuilder**（{@code getBuilderRaw} 都返回同一个
 * {@code this.builder}）。于是「先把四个 consumer 都取出来、再混着写顶点」的写法会坏在两点：
 * <ul>
 *   <li>四个引用其实是**同一个对象** —— 所有顶点都进了同一批；</li>
 *   <li>最后一次 {@code getBuffer} 把 {@code lastState} 定成了最后一个类型，而
 *       {@code endBatch(type)} 在「{@code lastState != type} 且用的是共享 builder」时会
 *       **直接 return 什么都不画**（javap 实测）。结果只有最后一次 {@code endBatch} 真的落笔，
 *       而且用的是**它那一张**贴图 —— 于是所有阶梯面（含斜段）都被贴上同一张图。</li>
 * </ul>
...
public final class EscalatorStepRenderer {
    ...
    private static void render(WorldRenderContext context) {
        ...
        //    分桶是必须的，原因见类注释：1.19.2 的自定义 RenderType 共用一个 BufferBuilder，
        //    混着写会把所有面都用最后那张贴图画出去。
