...
public final class EscalatorStepRenderer {
    ...
    private static void render(WorldRenderContext context) {
        ...
        // 必须「一个渲染类型画完并立刻刷出去，再画下一个」，不能像旧代码那样先把 up/down
        // 两个 buffer 都取出来：
        // 1.21 的 MultiBufferSource.BufferSource 对「没有登记在共享缓冲表里的自定义 RenderType」
        // 只会保留最后一个 BufferBuilder —— 再取第二个自定义类型时，它会先把上一个
        // endBatch 掉（build 之后 building 变 false）。于是旧代码里 up 的 builder 在取 down 的
        // 那一刻就已经失效，随后往 up 里写顶点会抛 IllegalStateException: Not building!，
        // 被外层 catch 吞掉 → 阶梯一帧都没画出来（而覆盖已把 MTR 原版台阶隐藏，于是阶梯"不动"
        // 且速度怎么调都没反应）；同时 push 的矩阵没机会 pop，原版 renderLevel 末尾的
        // 「Pose stack not empty」断言会失败 → 直接崩游戏。
        drawPass(level, buffers, upType, true,
                positions, boxes, frustum, cameraX, cameraY, cameraZ,
                lookX, lookY, lookZ, maxDistanceSq, poseStack, modelRenderer, partialTick);
        drawPass(level, buffers, downType, false,
                positions, boxes, frustum, cameraX, cameraY, cameraZ,
                lookX, lookY, lookZ, maxDistanceSq, poseStack, modelRenderer, partialTick);
    }

    /**
     * 只画某一种渲染类型（上行或下行贴图）的阶梯面，画完立刻 {@code endBatch}。
     *
     * <p>一趟只持有一个自定义 BufferBuilder 是必须的：见 {@link #render} 的说明。
     */
    private static void drawPass(ClientLevel level, MultiBufferSource.BufferSource buffers,
                                 RenderType type, boolean wantUp,
        ...
        for (int i = 0; i < positions.size(); i++) {
            ...
            // 这一趟只画本方向的面，另一方向交给另一趟。
            if (model == null || model.up() != wantUp) {
                continue;
            }
            ...
            int band = EscalatorStepTextures.bandFor(wantUp, stopped ? 0 : frameOf(speed, partialTick));
            model.setBand(band, EscalatorStepTextures.bandCount(wantUp));
