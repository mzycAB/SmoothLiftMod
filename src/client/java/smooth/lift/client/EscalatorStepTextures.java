...
public final class EscalatorStepTextures {
    ...
    private static int[] readFrameOrder(Resource resource, int frames) {
        ...
        try {
            Optional<AnimationMetadataSection> section =
                    resource.metadata().getSection(AnimationMetadataSection.TYPE);
            if (section.isEmpty()) {
                return order;
            }
            int[] cursor = {0};
            Map<Integer, Integer> temporary = new HashMap<>();
            // 1.21.4 起 AnimationMetadataSection 没有 forEachFrame 了（1.21.1 及以前是
            // forEachFrame((index, time) -> ...)），改成直接拿帧列表：
            // frames() 返回 Optional<List<AnimationFrame>>，每帧有 getIndex()/getTime()。
            for (AnimationFrame frame : section.get().frames().orElse(List.of())) {
                if (cursor[0] < order.length) {
                    // 1.21.4 的 AnimationFrame 访问器是 index()（1.21.1 及以前叫 getIndex()）。
                    temporary.put(cursor[0], frame.index());
                    cursor[0]++;
                }
            }
    ...
    private static void createRenderType(Strip strip) {
        ...
        RenderType.CompositeState state = RenderType.CompositeState.builder()
                ...
                .setTextureState(new RenderStateShard.TextureStateShard(strip.id, TriState.FALSE, false))
