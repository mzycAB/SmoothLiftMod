...
public class AudioSetupScreen extends Screen {
    ...
    private void buildUi() {
        ...
        // 顶部：返回 / 刷新
        addRenderableWidget(new Button(this.width / 2 - 100, 24, 96, 20,
                Component.literal("返回"), button -> onClose()));
        addRenderableWidget(new Button(this.width / 2 + 4, 24, 96, 20, Component.literal("刷新"), button -> {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            if (row.type == T_BUILTIN) {
                addRenderableWidget(new Button(this.width / 2 - BTN_W / 2, y, BTN_W, 20,
                        Component.literal(truncate(row.text, 28)), button -> bindAudio(row.id)));
            } else if (row.type == T_STORED) {
                addRenderableWidget(new Button(this.width / 2 - BTN_W / 2, y, BTN_W - 50, 20,
                        Component.literal(truncate(row.text, 22)), button -> bindAudio(row.id)));
                addRenderableWidget(new Button(this.width / 2 - BTN_W / 2 + BTN_W - 46, y, 46, 20,
                        Component.literal("删除"), button -> deleteAudio(row.id)));
            } else if (row.type == T_PENDING) {
                addRenderableWidget(new Button(this.width / 2 - BTN_W / 2, y, BTN_W, 20,
                        Component.literal(truncate(row.text, 28)), button -> importFolderAudio(row.id)));
            }
        }

        // 底部：解绑此扶梯
        addRenderableWidget(new Button(this.width / 2 - 60, this.height - BOTTOM_RESERVE + 6, 120, 20,
                Component.literal("解绑此扶梯"), button -> {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollY) {
        // 【1.19.2 API】GuiEventListener.mouseScrolled 是 3 个 double（1.20.2 起才加了横向 scrollX）。
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        // 【1.19.2 API】Screen.renderBackground 只有 1 个参数（PoseStack）。
        this.renderBackground(poseStack);
        super.render(poseStack, mouseX, mouseY, partialTick);

        GuiComponent.drawCenteredString(poseStack, this.font, this.title, this.width / 2, 10, 0xFFFFFF);

        // 只画「分段标题 / 灰色说明」这两种行 —— 可点行由按钮自己画文字。
        // 用裁剪区兜住，滚动到一半的行只露出可见部分，不会压到标题或底部。
        GuiComponent.enableScissor(0, LIST_TOP, this.width, listBottom);
        for (int i = 0; i < rows.size(); i++) {
            Row row = rows.get(i);
            ...
            GuiComponent.drawCenteredString(poseStack, this.font, Component.literal(row.text), this.width / 2, y + 6, color);
        }
        ...
        if (maxScroll > 0) {
            ...
            GuiComponent.fill(poseStack, barX, trackTop, barX + 4, trackTop + trackH, 0x40000000);
            int thumbH = Math.max(14, trackH * trackH / (rows.size() * ROW_H));
            int thumbY = trackTop + (trackH - thumbH) * scroll / maxScroll;
            GuiComponent.fill(poseStack, barX, thumbY, barX + 4, thumbY + thumbH, 0xFFAAAAAA);
        }
        ...
        GuiComponent.drawCenteredString(poseStack, this.font, Component.literal(info), this.width / 2, statusY,
