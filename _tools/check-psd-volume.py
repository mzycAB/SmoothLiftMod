...
if m:
    ...
    check(len(params) == 15, "★ PsdToneAudio 是 15 字段（1.20 的 13 + 本轮 2）",
          "得到 %d：%s" % (len(params), ", ".join(params)))
    check(params[-1] == "Integer arriveVolume",
          "★ arriveVolume 在末尾（追加式改包的标准姿势：老位置一个都没动）",
          "得到 %s" % params[-1])
    check(params[9] == "String midium" and params[10] == "Integer midiumWaitSeconds"
          and params[11] == "Integer midiumVolume",
...
if m4:
    ...
    check(len(args4) == 15, "★ 客户端构造实参 = 15", "得到 %d" % len(args4))
    if m:
        ...
        check(set(args4) == set(rec_names) and len(set(args4)) == 15,
              "15 个实参/参数名互不重复（重名会让上面那条断言失去分辨力）")
...
# 底部合并：两个框必须同一个 y（同排），且都在主界面
check(re.search(r"defaultVolumeInput = new EditBox\(this\.font, bottomLoudBoxX\(cx\), inputY",
                ui) is not None
      and re.search(r"closeWaitInput = new EditBox\(this\.font, bottomWaitBoxX\(cx\), inputY",
                    ui) is not None,
      "★★ 底部「默认音量」与「强制等待」用**同一个 y = inputY** ⇒ 合并成一横排")
...
m7 = re.search(r"BOTTOM_RESERVE_LIST\s*=\s*(\d+)\s*;", ui)
m8 = re.search(r"ROW_H\s*=\s*(\d+)\s*;", ui)
m9 = re.search(r"LIST_TOP\s*=\s*(\d+)\s*;", ui)
m10 = re.search(r"STATUS_Y_LIST\s*=\s*(-?\d+)\s*;", ui)
...
if all(x is not None for x in (m7, m8, m9, m10)):
    ...
    check(re.search(r"listTop = \(page == 3 \|\| page == 4\) \? LIST_TOP \+ ROW_H : LIST_TOP\s*;",
                    ui) is not None,
          "两个列表页（3/4）整体下移一行（给两列表头让位）")
    check(re.search(r"int reserve = \(page == 3 \|\| page == 4\) \? BOTTOM_RESERVE_LIST : BOTTOM_RESERVE",
                    ui) is not None,
          "★ 列表页用**自己那套**预留（不共用主界面的 BOTTOM_RESERVE，否则主界面白留）")
...
check(re.search(r"RIGHT_COL_FIRST_STORED_ROW\s*=\s*1\s*;", ui) is not None,
...
check(re.search(r"private int listRowCount\(\)\s*\{\s*return Math\.max\(pending\.size\(\), "
                r"stored\.size\(\) \+ RIGHT_COL_FIRST_STORED_ROW\);", ui) is not None,
      "★★ 右列多一行这件事**只写在 listRowCount() 一处**（4 处调用点全走它）")
check(ui.count("int rowCount = listRowCount();") == 4,
      "到站/进站两页各 2 处（rebuild + 滚动条）都改成 listRowCount()，共 4 处")
