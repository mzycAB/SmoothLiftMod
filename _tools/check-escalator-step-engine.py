...
if not jars:
    ...
    # 反混淆后的 jar 里 MC 字段名是 intermediary：LevelRenderer.visibleSections
    # → class_761.field_45616（1.20.4 intermediary）。dev jar 没反混淆时是 mojmap 名
    # visibleSections —— 两种都认，但必须命中一个。
    # ★ 这一条同时证明 access widener 的条目真的被解析成了这个字段（描述符写错的话
    #   Loom 的 validateAccessWidener 会先失败，而不是悄悄退化成「字段不存在」）。
    check(cp_utf8("field_45616") in rdr_cls or cp_utf8("visibleSections") in rdr_cls,
          "★ 渲染器确实引用了 LevelRenderer.visibleSections（借原版可见段集合）",
          "field_45616(intermediary) / visibleSections(mojmap)")
...
# ======================================================================
print()
print("===== 14) 【1.28】★ 遮挡剔除：借原版 visibleSections 判「这帧画不画」 =====")
# ======================================================================
...
#
# 判据来源：原版**没有**公开的查询接口（SectionOcclusionGraph 的可见性状态锁在
# private 的 currentGraph 里，update() 返回 void），所以借
# `LevelRenderer.visibleSections` —— 原版 renderSectionLayer 就是遍历它画地形的，
...
check(re.search(r"accessible\s+field\s+net/minecraft/client/renderer/LevelRenderer\s+"
                r"visibleSections\s+Lit/unimi/dsi/fastutil/objects/ObjectArrayList;", aw) is not None,
      "★ access widener 打开了 LevelRenderer.visibleSections（名字 + 描述符都要对）")
...
check("visibleSections" in refresh_body and "getOrigin()" in refresh_body,
      "集合来源是 visibleSections 里每个 RenderSection 的 origin")
