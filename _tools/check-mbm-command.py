...
# ======================================================================
print()
print("===== 5) 版本号 = 1.23.1204（沿用，未点名不改） =====")

gp = open(GP, encoding="utf-8").read()
got = re.search(r"mod_version=(.*)", gp).group(1).strip()
check(got == "1.23.1204", "gradle.properties mod_version = 1.23.1204", "得到 %s" % got)
...
if not jars:
    ...
    check(fm["version"] == "1.23.1204", "jar 内 fabric.mod.json version = 1.23.1204",
          "得到 %s" % fm["version"])
