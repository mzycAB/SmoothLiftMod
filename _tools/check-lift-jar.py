...
# 期望的 MC 版本从 gradle.properties 读，别写死 —— 本脚本要在 1.20.1 / 1.20.4 / 1.21.x 各工程通用。
# ★ 判据也不能写死前缀：1.20.x 工程声明 `~1.20.4`，1.21.x 工程声明 `=1.21.1`。
#   正确判据 = 「gradle.properties 里的 minecraft_version 满足 fabric.mod.json 声明的约束」，
#   即把声明里的前缀运算符（= / ~ / ^）去掉后，版本号应与 gradle.properties 一致。
MC_VER = None
...
DECLARED = (fm.get("depends", {}) or {}).get("minecraft")
if MC_VER and DECLARED:
    _d = str(DECLARED).strip()
    _norm = _d.lstrip("=~^").split()[0]
    check(_norm == MC_VER,
          "fabric.mod.json 依赖 minecraft（%s）与 gradle.properties 的 %s 对应" % (_d, MC_VER),
          "declared=%r / gradle=%r" % (_d, MC_VER))
else:
    check(False, "能同时读到 MC 版本（gradle.properties + fabric.mod.json）",
          "gradle=%r declared=%r" % (MC_VER, DECLARED))
