 ...
 * 以及【1.42】直梯的 `/lifthelp`、`/lifthelpspeed` 与【1.43】`/lifthelploud`
 * （【1.46】再带 `/lifthelpup|down|chime` 与 `lifthelploud up|down|door`、
 *  【1.47】`/lifthelpround`、【1.48】三项各自音量）的每个分支都真的可达。
 *
 * <p><b>Forge 1.20.1 移植说明</b>（本文件从 Fabric 参考工程原样搬来，只有下面这一处差别）：
 * Forge 侧没有「模块初始化器」这一层，指令注册走 {@code @SubscribeEvent onRegisterCommands(
 * RegisterCommandsEvent)}。为了让本工具仍能脱离游戏环境建树，Forge 版把注册体抽成了
 * package-private 的 {@code static registerCommands(CommandDispatcher<CommandSourceStack>)}，
 * `onRegisterCommands` 只做一次转发 —— 于是本文件一字不用改（它本来就是这么设计的）。
 *
