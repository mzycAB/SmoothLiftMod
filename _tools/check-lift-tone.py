...
# jar 名字随 mod_version 变（本脚本要在 4 个工程里通用），所以扫目录取最新一只，
# 而不是写死版本号 —— 否则换个版本号重编就会静默 SKIP 掉打包校验那一段。
_LIBS = os.path.join(ROOT, "build", "libs")
_CANDS = sorted(n for n in (os.listdir(_LIBS) if os.path.isdir(_LIBS) else [])
                if n.startswith("smooth-escalator-") and n.endswith(".jar") and "sources" not in n)
JAR = os.path.join(_LIBS, _CANDS[-1]) if _CANDS else os.path.join(_LIBS, "smooth-escalator-<未构建>.jar")
