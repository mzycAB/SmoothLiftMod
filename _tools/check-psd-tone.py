...
## 需求（用户原话归纳）
    ...
    5：列车屏蔽门开关门时可以播放自定义声音；石斧右键屏蔽门打开 UI（与直梯的石斧 UI 基本一致），
       有开门声音 / 关门声音列表和音量输入框（音量 1~1000）；音频导入文件夹 smoothlift_audio。
...
check("TOGGLE_SENTINEL" in screen and "SET_PSD_TONE_SWITCH_CHANNEL" in screen,
      "UI 有开关行（点=切换维度默认子开关）")
check("defaultVolumeInput" in screen and "toneVolumeInput" in screen
      and "SET_PSD_CHIME_VOLUME_CHANNEL" in screen and "SET_PSD_TONE_VOLUME_CHANNEL" in screen,
      "UI 有共用默认音量 + 单项音量两个输入框（= /pbmloud）")
...
check("总开关 " in screen and "强制等待 " in screen,
      "★【1.18】主界面读数行保留（总开关/默认音量/范围/强制等待），只是颜色由灰改白")
...
check("SET_PSD_MIDIUM_CHANNEL" in main and "SET_PSD_MIDIUM_CHANNEL" in screen,
      "石斧 UI 的「到站播放音频 / 等待几秒后播放」走 SET_PSD_MIDIUM_CHANNEL"
...
# ---- UI 版式（用户点名的那张图） ----
check('Component.literal("到站播放音频")' in screen,
      "UI 有「到站播放音频」按钮（点击进入选择列表）")
check('Button.builder(Component.literal("到站播放音频"), button -> {' in screen
      and re.search(r'Component\.literal\("到站播放音频"\)[\s\S]{0,120}?page = 3;', screen) is not None,
      "「到站播放音频」是**控件**且点它通向 page 3 = buildArrivalPage（不是画上去的标题）")
...
check(re.search(r'ROW_NAME_W = COL_W - 2 \* ROW_BTN_W - 2 \* ROW_BTN_GAP', screen) is not None,
...
_code = re.search(r"private static final int COL_W = (\d+);", screen)
...
check(len(_client_reads) == 13 and "new EscalatorSpeedData.PsdToneAudio(" in client,
      "★ 客户端 PSD_TONE_SYNC 接收端按**写入顺序**读回 13 个可选字段建 15 字段记录"
      "（顺序错 = 整条记录错位，而且不报错）"
      "　★【1.22】可选字段 11 → 13（多了到站/进站两个音量）",
...
check("SET_PSD_ARRIVE_CHANNEL" in main and "SET_PSD_ARRIVE_CHANNEL" in screen,
      "石斧 UI 的「进站播放音频 / 到站前秒数」走 SET_PSD_ARRIVE_CHANNEL（服务端 + UI 两端都在）")
...
# ---- UI 版式：进站播放音频那一行 ----
check('Component.literal("进站播放音频")' in screen, "UI 主界面有「进站播放音频」按钮")
check(re.search(r"LIST_TOP \+ 10 \+ ROW_H \* 4", screen) is not None,
      "★ 它排在到站播放音频（ROW_H*3）的**下面**一行（ROW_H*4）")
...
check(re.search(r"page == 3 \|\| page == 4", screen) is not None,
      "★ 进站报站页与到站播报页一样把列表整体下移一行为表头让位（listTop 那一档）")
