...
_HERE = Path(__file__).resolve().parent.parent          # 工程根


def _default_jar():
    """build/libs 下最新那只 jar（名字随 mod_version 变，别写死）。"""
    libs = _HERE / "build" / "libs"
    cands = sorted(p for p in libs.glob("smooth-escalator-*.jar") if "sources" not in p.name)
    return cands[-1] if cands else (libs / "smooth-escalator-<未构建>.jar")


JAR = Path(sys.argv[1]) if len(sys.argv) > 1 else _default_jar()
JAVAP = r"C:\Program Files\Eclipse Adoptium\jdk-21.0.11.10-hotspot\bin\javap.exe"


def _prop(key, default=""):
    p = _HERE / "gradle.properties"
    if p.is_file():
...
# 本脚本要在 Forge 1.20.1 / 1.20.4 两个工程里通用 —— 版本号与「是不是 1.20.4」
# 都从 gradle.properties 读（UI API 与网络层形状两边不同）。
EXPECT_VER = _prop("mod_version", "?")
IS_1204 = _prop("minecraft_version") == "1.20.4"
...
if IS_1204:
    # 49.x 的 messageBuilder 只吃 (类)，方向由「谁注册了谁」隐式决定，
    # 代码里不再出现 NetworkDirection —— 所以这条断言在 1.20.4 上应恒为 0。
    check("[网络] 49.x：messageBuilder 不再传 NetworkDirection（0 次）",
          len(dirs) == 0, f"实际 {len(dirs)}")
else:
...
check(f"[清单] Implementation-Version = {EXPECT_VER}",
      f"Implementation-Version: {EXPECT_VER}" in mf)
...
check("[客户端] LiftToneSetupScreen 走 Forge 的 sendToServer（1.20.1 是 Packets.CHANNEL.sendToServer，1.20.4 是 Packets.sendToServer）",
...
if pre.exists():
    jv = subprocess.run([JAVAP, "-p", "-cp", str(UNREOBF), "smooth.lift.client.LiftToneSetupScreen"],
                        capture_output=True, text=True, encoding="utf-8", errors="replace").stdout
    if IS_1204:
        check("[客户端] mouseScrolled 用 1.20.4 的 4 参数签名（未 reobf 类）",
              "mouseScrolled(double, double, double, double)" in jv,
              "未找到 4 参数 mouseScrolled")
        check("[客户端] 没有 1.20.1 的 3 参数 mouseScrolled 残留",
              "mouseScrolled(double, double, double)" not in jv)
    else:
